...
def cleanup_file_name(file_name):
    ...
    return file_name.replace(" ", "_").replace(":", "").replace("/", "").replace('%', '').replace('?', '').replace('=', '')
...
def main():
    ...
    parser.add_argument('--urls', '-u', nargs='+', required=True,
    ...
    # output epub file
    parser.add_argument('--epub', '-e', type=str, default=None, 
    ...
    cookies = {x: y for x, y in 
               [cookie.split("=") for cookie in args.cookies.split(";")]} if args.cookies else {}    
    ...
    for art in articles:
        ...
        for img in soup.find_all("img"):
            ...
            if not img_url.startswith("http"):
                img_url = url_to_base_path(url) + "/" + img_url
